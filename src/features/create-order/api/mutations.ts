...
function toCreateOrderPayload(input: CreateOrderFlowInput): CreateOrderPayload {
  return {
...
export function useCreateOrderFlowMutation() {
  ...
  return useMutation({
    mutationFn: async (input: CreateOrderFlowInput) => createOrder(toCreateOrderPayload(input)),
    onSuccess: (order) => {
