    ...
    <header className="grid gap-2.5 rounded-2xl border border-[var(--color-border)] bg-[rgba(27,33,48,0.9)] p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="grid gap-2">
          ...
          <p className="m-0 text-[color:var(--color-text-secondary)]">{t("pages.orderDetails.description")}</p>
          <p className="m-0 text-sm text-[color:var(--color-text-secondary)]">
            {t("pages.orderDetails.orderId", { orderId })}
          </p>
        </div>
        <Link
          to="/orders"
          className="inline-flex h-fit rounded-full border border-[rgba(107,164,255,0.35)] bg-[rgba(107,164,255,0.14)] px-2.5 py-1.5 text-xs font-semibold text-[color:var(--color-accent-light-blue)] transition-colors hover:bg-[rgba(107,164,255,0.2)]"
        >
